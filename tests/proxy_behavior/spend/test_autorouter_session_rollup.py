...
async def _turn(
    ...
    await db.execute_raw(
        ...
        key, session_id, router, router_type, model, at.isoformat(), tokens, spend, saved, covered, hit, ttl,
    )
...
async def test_return_misses_attribute_against_the_recorded_ttl(db):
    ...
    assert row["return_prefix_misses"] == 1
...
async def test_a_return_miss_with_no_recorded_ttl_stays_unattributed(db):
    ...
    assert row["return_prefix_misses"] == 0
...
async def test_a_hit_refreshes_the_models_cache_clock(db):
    ...
    row = await _row(db, key)
    assert row["return_prefix_misses"] == 2
    assert row["return_expired_misses"] == 0
    assert row["models"]["A"]["at"] == pytest.approx(_utc_epoch(T0 + timedelta(seconds=560)), abs=1)

...
SELECT
    ...
    COALESCE(SUM(return_prefix_misses), 0)::int AS return_prefix_misses,
...
def _benchmark_totals(row: _SessionAggRow) -> AutoRouterBenchmarkTotals:
    hits: Final = row.same_model_hits + row.first_visit_hits + row.return_hits
    ...
    return AutoRouterBenchmarkTotals(
        ...
        cache=AutoRouterCacheStats(
            ...
            hit_rate_pct=_pct(hits, row.covered_turns),
            ...
            return_misses_prefix_changed=row.return_prefix_misses,
            return_misses_unknown=max(return_misses - row.return_expired_misses - row.return_prefix_misses, 0),
...
def _summed_agg_row(rows: Sequence[_SessionAggRow]) -> _SessionAggRow:
    return _SessionAggRow(
        ...
        return_prefix_misses=sum(row.return_prefix_misses for row in rows),
